...
versions: any
recommendation: "Review and correct authorization configuration in Mendix applications, including entity access rules, role mappings and XPath constraints."
workaround: n/a
start: 2025-10-27
end:
timeline:
...
Because the client operates outside the control of the server, it must always be considered untrusted. Security must therefore be enforced at the server level.

Data exposure typically occurs when authorization rules within the application are not correctly implemented. Common causes include:

- missing or incorrect entity access rules
- incorrect module role mappings
- insufficient XPath constraints restricting data visibility
- overly permissive permissions for anonymous users
...
* Review permissions granted to newly registered or default users
...
[Menscan.io](https://www.menscan.io/) is a free online tool that helps identify potential configuration issues in Mendix applications. It operates as a middleware between the user and the Mendix application, highlighting common authorization and configuration mistakes during interaction with the application. Menscan does not store application data.

As an alternative, [MendixHunter](https://github.com/Jelle-DIVD/MendixHunter) is an open-source tool that can be run locally. MendixHunter can be used by researchers and security teams to analyse Mendix applications and support security assessments or research activities.
...
Organizations that would like additional information about this research effort can contact the DIVD CSIRT team at 2026-00003@csirt.divd.nl.
...
* https://www.mendix.com/trust/handling-security-findings-related-to-data-exposure/
